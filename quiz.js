...
function showQuestion() {
    ...
    if //show end screen
    (currentQuestion >= questions.length) { //wenn currentquestion länger/größer oder gleich als/wie Array(5) ist
...
function answer(currAnswer) { //Variable wird in die Klammer gegeben,da es 3 Antwort-Varianten gibt
    ...
    if (selectedQuestionNumber == question['right_answer']) { //wenn die selectedNr der Nr in right_answer übereinstimmt, ist die Antwort richtig
        document.getElementById('switchImg').src = './img/cardimg.png';
...
//shows next Array by click on button
